...
# Перцептрон: 1 входной слой, 3 скрытых, 1 выходной
class MLP3HL:
    def __init__(self, n_input, n_hidden1, n_hidden2, n_hidden3, n_output, learning_rate=0.05):
        ...
        self.w3 = np.random.randn(n_hidden2, n_hidden3) * 0.01
        self.b3 = np.zeros((1, n_hidden3))

        self.w4 = np.random.randn(n_hidden3, n_output)
    ...
    def backward(self, X, y):
        ...
        dz4 = self.a4 - y # Производная для кросс-энтропии со сигмоидой
    ...
    def update_parameters(self, grads):
        ...
        self.w4 -= self.learning_rate * grads["dw4"]
        self.b4 -= self.learning_rate * grads["db4"]
...
# Создание экземпляра MLP3HL
n_input = X.shape[1] # Количество признаков (2)
n_hidden1 = 15 # Нейроны в первом скрытом слое
n_hidden2 = 10 # Нейроны во втором скрытом слое
n_hidden3 = 8 # Нейроны в третьем скрытом слое
n_output = 1 # Выходной нейрон для бинарной классификации

mlp = MLP3HL(n_input, n_hidden1, n_hidden2, n_hidden3, n_output, learning_rate=0.05)
